...
% costJ = single number 
J1 = (-1/m) * sum( y .* log(H) + (1 - y) .* log(1 - H) ); % computing J without regularization

costRegularizationTerm = lambda/(2*m) * sum( theta(2:end).^2 );

J = J1 + costRegularizationTerm;
...
% grad = nx1 column vector
grad = (1/m) * grad + gradRegularizationTerm;
